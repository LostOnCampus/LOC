...
| 서버 OS | Ubuntu 24.04 LTS (Host) |
| 컨테이너 | Docker, Docker Compose |
| 웹 서버 | Apache 2.4 (Dockerized) |
| 서버 언어 | PHP 8 (Dockerized) |
...
1.  **`.env` 파일 준비:** 호스트 서버의 `/var/www/.env` 경로에 다음 예시와 같이 `.env` 파일을 생성하고 데이터베이스 접속 정보를 입력합니다.
    ```bash
    # 예시:
    # nano /var/www/.env
    ```
    ...
    LOC_DB_PASS=DB비밀번호
    ```
2.  **권한 설정 (선택 사항이나 보안 권장):**
    ```bash
    sudo chown root:www-data /var/www/.env
    sudo chmod 640 /var/www/.env
    ```
    Docker Compose 설정 시 이 파일이 컨테이너 내부의 `/var/www/.env`로 마운트됩니다.

## 배포 방법 (Docker Compose)

프로젝트를 Docker Compose를 사용하여 배포하는 방법입니다.

1.  **Docker 및 Docker Compose 설치:** 서버에 Docker와 Docker Compose가 설치되어 있어야 합니다.
2.  **프로젝트 클론:** GitHub에서 프로젝트를 클론합니다.
    ```bash
    git clone [Your Repository URL]
    cd LostOnCampus
    ```
3.  **`.env` 파일 설정:** 위 '환경 변수 설정' 섹션을 참조하여 호스트 서버의 `/var/www/.env` 파일을 준비합니다.
4.  **Docker Compose 실행:** 프로젝트 루트 디렉터리에서 다음 명령어를 실행하여 애플리케이션을 빌드하고 백그라운드에서 실행합니다.
    ```bash
    docker compose up -d --build
    ```
5.  **접속 확인:** 웹 브라우저에서 `http://<서버_공인IP>` 주소로 접속하여 애플리케이션이 정상적으로 동작하는지 확인합니다.
